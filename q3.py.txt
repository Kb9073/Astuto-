...
def find_overloaded_users(events):
	...
	feq_of_users = defaultdict(int)
	overloaded_users = defaultdict(int)
	time_range_per_user = defaultdict(int)

	for user_id , timestamp in events :
   	    freq_of_users[user_id] += value 
	    time_range_per_user[user_id] = max(timestamp , key = timestamp.get) - min(timestamp , key = timestamp.get) 
		if freq_of_users[user_id] >= 3 and  time_range_per_user[user_id] <= 10 :
			return (overloaded_users[user_id]) 
